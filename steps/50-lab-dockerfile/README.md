...
3. Copy `requirements.txt` and `app.py` in `myapp`
4. Run `cd myapp`
...
2. Copy `requirements.txt` in `/app/requirements.txt`
...
7. Add OCI metadata labels using `LABEL`: `org.opencontainers.image.authors` and `org.opencontainers.image.version`
8. Make sure the container will run the command `python app.py`

### Build the image

1. Build the docker image and name it `my_flask:1.0`
2. _(Optional — requires a Docker Hub account)_ Tag as `<dockerHubId>/my_flask:1.0` and push to Docker Hub
